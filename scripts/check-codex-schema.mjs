...
const missing = [...contract.methods, ...contract.responseFields].filter((token) => !generatedSchema.includes(`\"${token}\"`));
if (missing.length > 0) throw new Error(`Bundled Codex App Server schema is incompatible; missing: ${missing.join(", ")}`);
console.log(`Codex App Server schema covers ${contract.methods.length} methods and ${contract.responseFields.length} fields.`);
