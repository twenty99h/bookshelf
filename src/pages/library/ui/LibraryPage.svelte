<script lang="ts">
  ...
  async function importBook() {
    const snapshot = await session.mutate(() => commands.importPdf(), "Книга скопирована в личную библиотеку");
    if (snapshot) session.view = "library";
  }
  ...
  async function exportDraft(draftId: string) {
    await session.mutate(() => commands.exportDraft(draftId), "Черновая заметка экспортирована и убрана из очереди");
  }
    ...
    {:else if library}
      ...
      {#if settingsOpen}
        <LibrarySettingsPanel
          ...
          mutate={(command, success) => session.mutate(command, success)}
      ...
      {:else if view === "ideas"}
        ...
        {#if library.ideas.length === 0}<section
          ...
          >
            ...
            {run}
            {bookTitle}
            mutate={async (command, success) => {
              const snapshot = await session.mutate(command, success);
              if (!snapshot) throw new Error(session.error || "Команда не вернула снимок библиотеки");
              return snapshot;
            }}
      ...
      {:else}
        <StudyWorkspace {library} {activeBook} {run} onFeedback={(message) => (session.feedback = message)} />
      {/if}
    {/if}
  </div>
</WorkspaceShell>
