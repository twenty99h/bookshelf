<script lang="ts">
  ...
  async function saveOutline() {
    if (!readingBook) return;
    await run({ kind: "saveOutline", bookId: readingBook.id, outline: readingBook.outline ?? [] }, "Исправленное оглавление сохранено отдельно от PDF");
  }

  async function addOutlineItem(event: SubmitEvent) {
    event.preventDefault();
    if (!readingBook || !outlineTitle.trim()) return;
    readingBook.outline = [...(readingBook.outline ?? []), { id: `outline-${Date.now()}`, title: outlineTitle, page: outlinePage, parentId: null }];
    outlineTitle = "";
    await saveOutline();
  }
  ...
  async function exportDraft(draftId: string) {
    ...
    if (!path) return;
    try { library = await exportDraftMarkdown(draftId, path); feedback = "Черновая заметка экспортирована и убрана из очереди"; }
    catch (cause) { error = commandErrorMessage(cause); }
  }

  async function exportMaterial(materialId: string, title: string) {
    const path = await save({ defaultPath: `${title || "material"}.md`, filters: [{ name: "Markdown", extensions: ["md"] }] });
    if (!path) return;
    try { await exportMaterialMarkdown(materialId, path); feedback = "Markdown-файл сохранён; его изменения не затронут Bookshelf"; }
    catch (cause) { error = commandErrorMessage(cause); }
  }
...
<WorkspaceShell>
  {#snippet sidebar()}
    <button class="flex items-center gap-3 border-0 bg-transparent text-lg text-white" aria-label="Открыть библиотеку" onclick={() => navigate("library")}><span class="grid h-[42px] w-[35px] place-items-center rounded-[3px_9px_9px_3px] bg-[#b8d94a] font-display text-[23px] text-ink shadow-[inset_3px_0_#91ab36]" aria-hidden="true">B</span><b>Bookshelf</b></button>
    <nav class="mt-3.5 flex gap-1 overflow-x-auto min-[901px]:mt-[46px] min-[901px]:grid" aria-label="Основная навигация">
      <NavigationItem label="Библиотека" active={view === "library"} count={library?.books.length ?? 0} onclick={() => navigate("library")} />
      <NavigationItem label="Очередь разбора" active={view === "queue"} count={library?.drafts.length ?? 0} onclick={() => navigate("queue")} />
      <NavigationItem label="Идеи" active={view === "ideas"} count={library?.ideas.length ?? 0} onclick={() => navigate("ideas")} />
      <NavigationItem label="Изучение" active={view === "study"} onclick={() => navigate("study")} />
    </nav>
    <button class="absolute right-[18px] top-3.5 min-h-11 rounded-lg border-0 bg-transparent px-3 py-2.5 text-left text-spine-muted hover:bg-spine-raised hover:text-white min-[901px]:static min-[901px]:mt-auto" aria-expanded={settingsOpen} onclick={() => settingsOpen = !settingsOpen}>Настройки</button>
    <p class="mt-[18px] hidden px-2 text-[11px] leading-[1.4] text-[#aab2b9] min-[901px]:block"><i class="mr-1 inline-block size-[7px] rounded-full bg-[#b8d94a]"></i> Данные остаются на этом компьютере</p>
  {/snippet}
    {#if loading}
    ...
    {:else if library}
      <PageHeader eyebrow="Личная библиотека" title={view === "library" ? "Рабочий стол читателя" : view === "queue" ? "Очередь разбора" : view === "ideas" ? "Идеи книги" : "Ритм изучения"}>
        {#snippet actions()}<Button variant="primary" onclick={importBook} disabled={busy}>Импортировать PDF</Button>{/snippet}
      </PageHeader>
      ...
      {:else}
        ...
        {#if library.sessions.length}<section class="stack">{#each library.sessions as session}<article class="session"><div><b>{session.intention}</b><small>{session.status === "planned" ? "Запланирован" : session.status === "active" ? "Сеанс идёт" : session.status}</small></div>{#if session.status === "planned"}<div class="session-actions"><Button variant="primary" onclick={() => run({ kind: "startSession", sessionId: session.id }, "Сеанс начат; изменение долга будет измерено отсюда")}>Начать сеанс</Button><select aria-label="Решение по пропущенному сеансу" bind:value={missedSessionStatus}><option value="moved">Перенести</option><option value="replaced">Заменить</option><option value="cancelled">Отменить</option></select><input aria-label="Причина решения" bind:value={missedSessionReason} placeholder="Почему план изменился" /><Button onclick={() => run({ kind: "resolveSession", sessionId: session.id, status: missedSessionStatus, reason: missedSessionReason }, "Решение по сеансу сохранено без штрафа")}>Сохранить решение</Button></div>{:else if session.status === "active"}<Button onclick={() => completeSession(session.id)}>Завершить сеанс</Button>{/if}</article>{/each}</section>{/if}
...
<style>
  ...
  h3, p { margin-top: 0; }
  .eyebrow { margin: 0 0 7px; color: #66717a; font-size: 11px; font-weight: 800; letter-spacing: .11em; text-transform: uppercase; }
  .feedback { min-height: 24px; margin: -8px 0 18px; color: var(--color-success); font-size: 13px; }
  .settings-panel { display: grid; grid-template-columns: .8fr 1.2fr; gap: 34px; margin-bottom: 24px; border: 1px solid var(--color-rule); border-radius: 12px; background: var(--color-paper-raised); padding: 24px; box-shadow: var(--shadow-paper); }
  .settings-panel p, .muted { color: var(--color-ink-muted); line-height: 1.55; }
  .form-actions, .card-actions { display: flex; flex-wrap: wrap; align-items: center; gap: 9px; margin-top: 10px; }
  .bookmark { display: grid; grid-template-columns: minmax(0, 1fr) auto 92px; align-items: center; gap: 24px; overflow: hidden; margin-bottom: 30px; border-radius: 4px 13px 13px 4px; background: #eaf1c8; padding: 22px 26px 22px 34px; box-shadow: inset 7px 0 #b8d94a; }
  .bookmark span, .debt small { display: block; margin-bottom: 5px; color: #53602d; font-size: 10px; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; }
  .bookmark strong { font-family: Georgia, serif; font-size: 22px; font-weight: 500; }.bookmark p { margin: 6px 0 0; color: #606a50; font-size: 13px; }
  ...
  blockquote { margin: 14px 0; border-left: 3px solid #b8d94a; padding: 6px 0 6px 16px; color: #45515a; line-height: 1.55; } details { margin-top: 14px; border-top: 1px solid var(--color-rule); padding-top: 12px; } summary { cursor: pointer; font-weight: 700; } pre { overflow: auto; white-space: pre-wrap; font: 12px/1.55 ui-monospace, monospace; }
  .section-head { align-items: center; }.section-head p { color: var(--color-ink-muted); }.section-head form { width: min(460px, 50%); }.study-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px; }
